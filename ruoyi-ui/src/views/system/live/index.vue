<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="liveList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="支付金额" align="center" prop="totalPay" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        userId: [
          { required: true, message: "入住用户号不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除入住管理编号为"' + liveIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/live/export', {
        ...this.queryParams
